...
public class SystemDestructionConfig {
    ...
    @Bean
    @JobScope // 리스너에서 잡 파라미터를 주입받기 위해 JobScope 설정 (JobExecutionListener 는 Job Execution 과 생명주기를 함께하기에, JobScope 사용)
    public JobExecutionListener systemTerminationListener(
            @Value("#{jobParameters['terminationType']}") String terminationType // 잡 파라미터 주입
    ) {
