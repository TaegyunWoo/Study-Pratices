<!-- TOC -->
* [태스크릿 지향 처리](#태스크릿-지향-처리)
  * [태스크릿 지향 처리 특징](#태스크릿-지향-처리-특징)
  * [예시 코드](#예시-코드)
* [청크 지향 처리](#청크-지향-처리)
  * [청크 지향 처리 특징](#청크-지향-처리-특징)
  * [`ItemReader` : 데이터 읽기](#itemreader--데이터-읽기)
  * [`ItemProcessor` : 데이터 가공 및 처리](#itemprocessor--데이터-가공-및-처리)
  * [`ItemWriter` : 데이터 쓰기](#itemwriter--데이터-쓰기)
  * [Reader-Processer-Writer 패턴](#reader-processer-writer-패턴)
  * [예시 코드](#예시-코드-1)
  * [청크 지향 처리 흐름도](#청크-지향-처리-흐름도)
  * [청크 지향 처리 반복 흐름도](#청크-지향-처리-반복-흐름도)
* [JobParameters](#jobparameters)
  * [JobParameters 란](#jobparameters-란)
  * [다양한 JobParameters 전달 방법](#다양한-jobparameters-전달-방법)
    * [커맨드라인에서 잡 파라미터 전달하기](#커맨드라인에서-잡-파라미터-전달하기)
    * [기본 파라미터 표기법의 한계](#기본-파라미터-표기법의-한계)
    * [JSON 형식으로 JobParameters 전달하기](#json-형식으로-jobparameters-전달하기)
  * [JobLauncherApplicationRunner](#joblauncherapplicationrunner)
    * [JobLauncherApplicationRunner 란?](#joblauncherapplicationrunner-란)
    * [JobLauncherApplicationRunner 가 수행하는 처리 과정](#joblauncherapplicationrunner-가-수행하는-처리-과정)
  * [Job Parameter 를 가져오는 방법](#job-parameter-를-가져오는-방법)
    * [`@Value` 사용](#value-사용)
    * [Context 에서 가져오기](#context-에서-가져오기)
  * [Job Parameter 값 검증](#job-parameter-값-검증)
    * [직접 Validator 구현하는 방법](#직접-validator-구현하는-방법)
      * [1. JobParametersValidator 구현](#1-jobparametersvalidator-구현)
      * [2. JobParametersValidator 를 Job에 등록](#2-jobparametersvalidator-를-job에-등록)
    * [기본 Validator 사용](#기본-validator-사용)
      * [예시 코드](#예시-코드-2)
* [Job 과 Step 의 Scope](#job-과-step-의-scope)
  * [JobScope 와 StepScope 의 라이프사이클](#jobscope-와-stepscope-의-라이프사이클)
  * [`@JobScope`](#jobscope)
    * [RestAPI로 요청을 받아서, 동적으로 Batch 를 실행하는 예시](#restapi로-요청을-받아서-동적으로-batch-를-실행하는-예시)
  * [`@StepScope`](#stepscope)
    * [예시 코드](#예시-코드-3)
  * [JobScope 와 StepScope 사용시 주의사항](#jobscope-와-stepscope-사용시-주의사항)
    * [1. 프록시 대상의 타입이 클래스인 경우, 반드시 상속 가능한 클래스여야 한다.](#1-프록시-대상의-타입이-클래스인-경우-반드시-상속-가능한-클래스여야-한다)
    * [2. Step 빈에는 `@StepScope` 와 `@JobScope` 를 사용하면 안된다.](#2-step-빈에는-stepscope-와-jobscope-를-사용하면-안된다)
      * [2-1. Step 빈에 `@StepScope` 를 설정하는 경우](#2-1-step-빈에-stepscope-를-설정하는-경우)
      * [2-2. Step 빈에 `@JobScope` 를 설정하는 경우](#2-2-step-빈에-jobscope-를-설정하는-경우)
    * [JobScope와 StepScope의 동작원리](#jobscope와-stepscope의-동작원리)
      * [JobScope 구조적 흐름](#jobscope-구조적-흐름)
      * [StepScope 구조적 흐름](#stepscope-구조적-흐름)
      * [JobScope와 StepScope의 동작원리의 핵심](#jobscope와-stepscope의-동작원리의-핵심)
    * [SpEL 표현식과 JobContext/StepContext 간의 관계](#spel-표현식과-jobcontextstepcontext-간의-관계)
  * [컴파일 시점에 없는 Job Parameter 값을 참조하는 방법](#컴파일-시점에-없는-job-parameter-값을-참조하는-방법)
  * [JobScope 와 StepScope 빈에서 ExecutionContext 의 데이터에 접근하는 방법](#jobscope-와-stepscope-빈에서-executioncontext-의-데이터에-접근하는-방법)
    * [ExecutionContext 란?](#executioncontext-란)
    * [ExecutionContext 에서 데이터 꺼내기](#executioncontext-에서-데이터-꺼내기)
* [Spring Batch Listener](#spring-batch-listener)
  * [Spring Batch Listener 란?](#spring-batch-listener-란)
  * [Spring Batch가 제공하는 주요 Listener 인터페이스](#spring-batch가-제공하는-주요-listener-인터페이스)
    * [JobExecutionListener](#jobexecutionlistener)
    * [StepExecutionListener](#stepexecutionlistener)
    * [ChunkListener](#chunklistener)
    * [ItemReadListener, ItemProcessListener, ItemWriteListener](#itemreadlistener-itemprocesslistener-itemwritelistener)
    * [각 리스너들의 호출 시점](#각-리스너들의-호출-시점)
    * [리스너 활용 예시](#리스너-활용-예시)
  * [배치 리스너 구현 방법](#배치-리스너-구현-방법)
    * [구현 방법 1) 전용 리스너 인터페이스를 직접 구현하는 방식](#구현-방법-1-전용-리스너-인터페이스를-직접-구현하는-방식)
    * [구현 방법 2) 리스너 특화 애너테이션을 사용하는 방식](#구현-방법-2-리스너-특화-애너테이션을-사용하는-방식)
  * [JobExecutionListener 와 ExecutionContext 를 활용한 동적 데이터 전달](#jobexecutionlistener-와-executioncontext-를-활용한-동적-데이터-전달)
    * [예시 코드](#예시-코드-4)
  * [JobParameter 가 아닌 ExecutionContext 를 사용하는 이유](#jobparameter-가-아닌-executioncontext-를-사용하는-이유)
    * [ExecutionContext 사용시 주의사항](#executioncontext-사용시-주의사항)
      * [잘못된 코드](#잘못된-코드)
    * [올바른 코드](#올바른-코드)
  * [보다 간결하게 ExecutionContext 가져오기](#보다-간결하게-executioncontext-가져오기)
    * [ExecutionContextPromotionListener 란?](#executioncontextpromotionlistener-란)
    * [ExecutionContextPromotionListener 사용시 주의사항](#executioncontextpromotionlistener-사용시-주의사항)
  * [Listener 와 `@JobScope`, `@StepScope` 통합](#listener-와-jobscope-stepscope-통합)
  * [리스너를 효과적으로 다루는 방법](#리스너를-효과적으로-다루는-방법)
    * [적절한 리스너 사용](#적절한-리스너-사용)
    * [예외 처리는 신중하게](#예외-처리는-신중하게)
    * [단일 책임 원칙 준수](#단일-책임-원칙-준수)
      * [성능 최적화) 실행 빈도 고려](#성능-최적화-실행-빈도-고려)
    * [성능 최적화) 리소스 사용 최소화](#성능-최적화-리소스-사용-최소화)
      * [고정 길이 형식의 파일 읽기](#고정-길이-형식의-파일-읽기)
      * [정규식으로 파일 읽기](#정규식으로-파일-읽기)
* [파일 기반 배치 처리](#파일-기반-배치-처리)
  * [FlatFileItemReader 와 FlatFileItemWriter](#flatfileitemreader-와-flatfileitemwriter)
    * [FlatFile 이란?](#flatfile-이란)
    * [FlatFileItemReader](#flatfileitemreader)
      * [`FlatFileItemReader.doRead()`](#flatfileitemreaderdoread)
      * [문자열을 쪼개서 도메인 객체로 매핑하기: `DefaultLineMapper`](#문자열을-쪼개서-도메인-객체로-매핑하기-defaultlinemapper)
      * [FlatFileItemReader 원리 정리](#flatfileitemreader-원리-정리)
      * [예시코드 1) 구분자로 토큰화해서 읽기](#예시코드-1-구분자로-토큰화해서-읽기)
      * [예시코드 2) 고정 길이로 토큰화해서 읽기](#예시코드-2-고정-길이로-토큰화해서-읽기)
      * [예시코드 3) 정규식으로 토큰화해서 읽기](#예시코드-3-정규식으로-토큰화해서-읽기)
      * [예시코드 4) 조건에 따라 다른 Tokenizer 와 FieldSetMapper 사용하기](#예시코드-4-조건에-따라-다른-tokenizer-와-fieldsetmapper-사용하기)
      * [예시코드 5) 여러 파일을 읽어서 배치 실행하기](#예시코드-5-여러-파일을-읽어서-배치-실행하기)
      * [번외) Record 클래스 객체로 FieldSetMapper 적용하기](#번외-record-클래스-객체로-fieldsetmapper-적용하기)
* [FlatFileItemWriter](#flatfileitemwriter)
  * [FlatFileItemWriter 란?](#flatfileitemwriter-란)
    * [1단계: FieldExtractor (필드 추출)](#1단계-fieldextractor-필드-추출)
    * [2단계 - 구분자로 문자열 합치기: `LineAggregator` -> `DelimitedLineAggregator`](#2단계---구분자로-문자열-합치기-lineaggregator---delimitedlineaggregator)
    * [예시코드](#예시코드)
    * [2단계 - 커스텀 포맷 형식으로 문자열 합치기: `LineAggregator` -> `FormatterLineAggregator`](#2단계---커스텀-포맷-형식으로-문자열-합치기-lineaggregator---formatterlineaggregator)
  * [FlatFileItemWriter 의 롤백 전략: 버퍼링을 통한 안전한 파일 쓰기](#flatfileitemwriter-의-롤백-전략-버퍼링을-통한-안전한-파일-쓰기)
  * [파일 쓰기와 OS 캐시: `forceSync()`](#파일-쓰기와-os-캐시-forcesync)
  * [대용량 파일의 분할 처리: `MultiResourceItemWriter`](#대용량-파일의-분할-처리-multiresourceitemwriter)
    * [예시 코드](#예시-코드-5)
* [FlatFileItemReader 와 FlatFileItemWriter 실전 예제](#flatfileitemreader-와-flatfileitemwriter-실전-예제)
* [JSON 파일 읽고 쓰기 예제](#json-파일-읽고-쓰기-예제)
* [관계형 데이터베이스 읽고 쓰기](#관계형-데이터베이스-읽고-쓰기)
  * [관계형 DB 읽기](#관계형-db-읽기)
    * [`JdbcCursorItemReader` : 커서 기반 처리](#jdbccursoritemreader--커서-기반-처리)
      * [`JdbcCursorItemReader` 구성요소](#jdbccursoritemreader-구성요소)
      * [`JdbcCursorItemReader` 예시 코드](#jdbccursoritemreader-예시-코드)
      * [JDBC 드라이버의 내부 최적화](#jdbc-드라이버의-내부-최적화)
      * [커서의 연속성](#커서의-연속성)
      * [스냅샷 읽기](#스냅샷-읽기)
      * [JdbcCursorItemReader 의 SQL `ORDER BY` 설정](#jdbccursoritemreader-의-sql-order-by-설정)
    * [`JdbcPagingItemReader` : 페이징 기반 처리](#jdbcpagingitemreader--페이징-기반-처리)
      * [`JdbcPagingItemReader` 구성요소](#jdbcpagingitemreader-구성요소)
      * [`JdbcPagingItemReaderBuilder` 의 쿼리 설정 메서드](#jdbcpagingitemreaderbuilder-의-쿼리-설정-메서드)
      * [`JdbcPagingItemReader` 예시 코드](#jdbcpagingitemreader-예시-코드)
  * [관계형 DB 쓰기](#관계형-db-쓰기)
    * [DB Insert 방식; `일반 INSERT` vs `Multi-value INSERT` vs `Batch Update`](#db-insert-방식-일반-insert-vs-multi-value-insert-vs-batch-update)
      * [`일반 INSERT` : 매 쿼리마다 네트워크 패킷 발생](#일반-insert--매-쿼리마다-네트워크-패킷-발생)
      * [`Multi-Value INSERT` : 여러 값을 하나의 쿼리로 한 번에 전송](#multi-value-insert--여러-값을-하나의-쿼리로-한-번에-전송)
      * [`Batch Update`](#batch-update)
    * [`JdbcBatchItemWriter` 구성요소](#jdbcbatchitemwriter-구성요소)
    * [JdbcBatchItemWriter 내부 동작 구조](#jdbcbatchitemwriter-내부-동작-구조)
    * [JdbcPagingItemReader와 JdbcBatchItemWriter 예제 코드](#jdbcpagingitemreader와-jdbcbatchitemwriter-예제-코드)
  * [JpaCursorItemReader](#jpacursoritemreader)
    * [JpaCursorItemReader 구성요소](#jpacursoritemreader-구성요소)
    * [JpaCursorItemReader 실행 흐름](#jpacursoritemreader-실행-흐름)
    * [JpaCursorItemReader 예시 코드](#jpacursoritemreader-예시-코드)
  * [JpaPagingItemReader](#jpapagingitemreader)
    * [JpaPagingItemReader 구성요소](#jpapagingitemreader-구성요소)
    * [JpaPagingItemReader 사용시 주의사항](#jpapagingitemreader-사용시-주의사항)
    * [JpaPagingItemReader 예시 코드](#jpapagingitemreader-예시-코드)
  * [JpaItemWriter](#jpaitemwriter)
    * [JpaItemWriter 사용시 주의사항](#jpaitemwriter-사용시-주의사항)
    * [예제 코드](#예제-코드)
* [위임 ItemWriter 와 ItemReader](#위임-itemwriter-와-itemreader)
  * [CompositeItemReader](#compositeitemreader)
    * [CompositeItemReader 가 유용한 경우](#compositeitemreader-가-유용한-경우)
  * [CompositeItemWriter](#compositeitemwriter)
    * [CompositeItemWriter 가 유용한 경우](#compositeitemwriter-가-유용한-경우)
    * [여러 시스템에 쓰기 작업을 할때의 트랜잭션 관리](#여러-시스템에-쓰기-작업을-할때의-트랜잭션-관리)
  * [ClassifierCompositeItemWriter](#classifiercompositeitemwriter)
    * [ClassifierCompositeItemWriter 예시코드](#classifiercompositeitemwriter-예시코드)
* [ItemStream](#itemstream)
  * [ItemStream 의 역할](#itemstream-의-역할)
  * [ItemStream : 자원 초기화 및 해제](#itemstream--자원-초기화-및-해제)
  * [ItemStream : 메타 데이터 관리 및 상태 추적](#itemstream--메타-데이터-관리-및-상태-추적)
    * [open() : 저장된 실행 정보 복원](#open--저장된-실행-정보-복원)
    * [update() : 상태 저장](#update--상태-저장)
    * [`CompositeItemReader` 의 ItemStream 처리](#compositeitemreader-의-itemstream-처리)
    * [ItemStream 수동 설정](#itemstream-수동-설정)
    * [ItemStream 자동 등록 메커니즘](#itemstream-자동-등록-메커니즘)
    * [ItemStream 을 구현한 컴포넌트를 `@StepScope` 로 사용시 주의사항](#itemstream-을-구현한-컴포넌트를-stepscope-로-사용시-주의사항)
* [ItemProcessor 의 데이터 처리 방식](#itemprocessor-의-데이터-처리-방식)
  * [1) `null` 반환을 통한 데이터 필터링 vs 데이터 검증을 통한 실패 처리](#1-null-반환을-통한-데이터-필터링-vs-데이터-검증을-통한-실패-처리)
    * [데이터 필터링 예시 코드](#데이터-필터링-예시-코드)
    * [ValidatingItemProcessor](#validatingitemprocessor)
  * [2) 데이터 변환](#2-데이터-변환)
    * [데이터 변환 예시 코드](#데이터-변환-예시-코드)
  * [3) 데이터 보강](#3-데이터-보강)
    * [데이터 보강 예시 코드](#데이터-보강-예시-코드)
    * [`ItemWriteListener.beforeWrite()`를 통한 외부 시스템 통신 최적화](#itemwritelistenerbeforewrite를-통한-외부-시스템-통신-최적화)
  * [CompositeItemProcessor : 여러 ItemProcessor 사용하기](#compositeitemprocessor--여러-itemprocessor-사용하기)
  * [ClassifierCompositeItemProcessor : 아이템을 처리할 ItemProcessor 라우팅](#classifiercompositeitemprocessor--아이템을-처리할-itemprocessor-라우팅)
* [Spring Batch 의 Fault Tolerant (내결함성)](#spring-batch-의-fault-tolerant-내결함성)
  * [Retry](#retry)
    * [RetryTemplate : Retry 의 핵심 컴포넌트](#retrytemplate--retry-의-핵심-컴포넌트)
      * [RetryTemplate 의 관리 범위와 `ChunkProcessor`](#retrytemplate-의-관리-범위와-chunkprocessor)
    * [RetryPolicy : 재시도 정책 관리](#retrypolicy--재시도-정책-관리)
      * [SimpleRetryPolicy](#simpleretrypolicy)
    * [Input Chunk 재활용을 통한, 내결함성 최적화](#input-chunk-재활용을-통한-내결함성-최적화)
    * [Retry 예제 코드](#retry-예제-코드)
    * [ItemProcessor 에서의 Retry 횟수 관리](#itemprocessor-에서의-retry-횟수-관리)
    * [Retry 최적화](#retry-최적화)
      * [`processorNonTransactional()` 없는 기본 동작시](#processornontransactional-없는-기본-동작시)
      * [`processorNonTransactional()` 설정시](#processornontransactional-설정시)
      * [`processorNonTransactional()` 설정 예시](#processornontransactional-설정-예시)
      * [`processorNonTransactional()` 사용시 주의사항](#processornontransactional-사용시-주의사항)
    * [ItemWriter 에서의 Retry](#itemwriter-에서의-retry)
    * [ItemWriter 에서의 Retry 횟수 관리](#itemwriter-에서의-retry-횟수-관리)
    * [다양한 재시도 정책(Retry Policy) 적용하기](#다양한-재시도-정책retry-policy-적용하기)
    * [BackOffPolicy](#backoffpolicy)
  * [Skip](#skip)
    * [예시 코드](#예시-코드-6)
    * [SkipPolicy](#skippolicy)
    * [Skip 실전 예제](#skip-실전-예제)
    * [ItemWriter 에서의 Skip 메커니즘](#itemwriter-에서의-skip-메커니즘)
      * [스캔모드에서의 Skip 프로세스](#스캔모드에서의-skip-프로세스)
    * [Skip 사용시의 주의점](#skip-사용시의-주의점)
      * [스캔 모드 동작 예시 코드](#스캔-모드-동작-예시-코드)
    * [ItemReader 에서의 Skip 메커니즘](#itemreader-에서의-skip-메커니즘)
      * [ItemReader 에서의 Skip 예시 코드](#itemreader-에서의-skip-예시-코드)
    * [SkipListener : 건너뛴 아이템 추적 및 후속 처리](#skiplistener--건너뛴-아이템-추적-및-후속-처리)
      * [SkipListener 예제 코드](#skiplistener-예제-코드)
    * [`noRollback()` 을 통한 성능 최적화](#norollback-을-통한-성능-최적화)
      * [`noRollback` : ItemReader 에서의 동작 메커니즘](#norollback--itemreader-에서의-동작-메커니즘)
      * [`noRollback` : ItemProcessor 에서의 동작 메커니즘](#norollback--itemprocessor-에서의-동작-메커니즘)
      * [`noRollback` : ItemWriter 에서의 동작 메커니즘](#norollback--itemwriter-에서의-동작-메커니즘)
* [Job과 메타데이터 저장소](#job과-메타데이터-저장소)
  * [개요](#개요)
  * [Job 의 핵심 도메인](#job-의-핵심-도메인)
    * [JobInstance (Job의 논리적 실행 단위)](#jobinstance-job의-논리적-실행-단위)
    * [JobParameters (JobInstance를 식별하는 매개변수)](#jobparameters-jobinstance를-식별하는-매개변수)
    * [JobExecution (JobInstance의 실제 실행 시도/이력)](#jobexecution-jobinstance의-실제-실행-시도이력)
      * [JobExecution 의 주요 실행 정보 - BatchStatus](#jobexecution-의-주요-실행-정보---batchstatus)
  * [JobInstance, JobParameters, JobExecution 으로 알아보는 Spring Batch 의 핵심 실행 원칙](#jobinstance-jobparameters-jobexecution-으로-알아보는-spring-batch-의-핵심-실행-원칙)
    * [JobInstance 의 재실행 제한](#jobinstance-의-재실행-제한)
    * [동일 작업 여러 번 실행하기](#동일-작업-여러-번-실행하기)
      * [JobParametersIncrementer](#jobparametersincrementer)
      * [RunIdIncrementer](#runidincrementer)
    * [identifying 속성](#identifying-속성)
    * [restartable 설정](#restartable-설정)
  * [Step 의 핵심 도메인](#step-의-핵심-도메인)
    * [StepExecution (Step의 실제 실행 시도/이력)](#stepexecution-step의-실제-실행-시도이력)
      * [StepExecution 의 주요 실행 정보 - BatchStatus](#stepexecution-의-주요-실행-정보---batchstatus)
      * [실패와 재시작시, StepExecution 의 동작 방식](#실패와-재시작시-stepexecution-의-동작-방식)
      * [StepExecution 의 전략적 활용 예시](#stepexecution-의-전략적-활용-예시)
  * [ExecutionContext : 상태 저장소](#executioncontext--상태-저장소)
    * [ItemStream 과 ExecutionContext](#itemstream-과-executioncontext)
  * [배치 메타데이터 테이블 구조](#배치-메타데이터-테이블-구조)
    * [주요 메타데이터 테이블](#주요-메타데이터-테이블)
      * [`BATCH_JOB_INSTANCE` 테이블](#batch_job_instance-테이블)
      * [`BATCH_JOB_EXECUTION` 테이블](#batch_job_execution-테이블)
      * [`BATCH_JOB_EXECUTION_PARAMS` 테이블](#batch_job_execution_params-테이블)
      * [`BATCH_STEP_EXECUTION` 테이블](#batch_step_execution-테이블)
      * [`BATCH_JOB_EXECUTION_CONTEXT` 테이블 , `BATCH_STEP_EXECUTION_CONTEXT` 테이블](#batch_job_execution_context-테이블--batch_step_execution_context-테이블)
  * [생명주기 - JobInstance, JobExecution, Job 수준 ExecutionContext](#생명주기---jobinstance-jobexecution-job-수준-executioncontext)
    * [JobInstance](#jobinstance)
    * [JobExecution](#jobexecution)
    * [Job 수준 ExecutionContext](#job-수준-executioncontext)
* [Spring Batch 의 AutoConfiguration](#spring-batch-의-autoconfiguration)
  * [BatchAutoConfiguration](#batchautoconfiguration)
    * [SpringBootBatchConfiguration](#springbootbatchconfiguration)
  * [SpringBootBatchConfiguration 와 DefaultBatchConfiguration](#springbootbatchconfiguration-와-defaultbatchconfiguration)
    * [DefaultBatchConfiguration 란?](#defaultbatchconfiguration-란)
  * [`@EnableBatchProcessing`](#enablebatchprocessing)
  * [DefaultBatchConfiguration 상속 / `@EnableBatchProcessing` 애너테이션 방식의 커스터마이징 사용시, 주의사항](#defaultbatchconfiguration-상속--enablebatchprocessing-애너테이션-방식의-커스터마이징-사용시-주의사항)
  * [Bean으로 직접 등록 : 가장 추천되는 컴포넌트 커스터마이징 방법](#bean으로-직접-등록--가장-추천되는-컴포넌트-커스터마이징-방법)
  * [`@BatchXXX` 애너테이션의 역할](#batchxxx-애너테이션의-역할)
    * [TransactionManager 분리하기](#transactionmanager-분리하기)
    * [**트랜잭션 매니저 분리시 주의사항**](#트랜잭션-매니저-분리시-주의사항)
* [커맨드라인에서의 ExitCode 조작](#커맨드라인에서의-exitcode-조작)
  * [ExitCode 기본 동작](#exitcode-기본-동작)
    * [ExitCode 변경하기](#exitcode-변경하기)
  * [JobExecutionExitCodeGenerator](#jobexecutionexitcodegenerator)
  * [세밀한 ExitCode 제어](#세밀한-exitcode-제어)
    * [1. ExitStatus 설정](#1-exitstatus-설정)
    * [2. JobListener 감지](#2-joblistener-감지)
* [REST API 를 통한 Job 실행과 JopOperator](#rest-api-를-통한-job-실행과-jopoperator)
  * [환경 구성 및 API 작성](#환경-구성-및-api-작성)
    * [1. 의존성](#1-의존성)
    * [2. application.yml](#2-applicationyml)
    * [3. Main Application](#3-main-application)
    * [4. 컨트롤러 작성](#4-컨트롤러-작성)
    * [5. Batch 비동기 실행 처리](#5-batch-비동기-실행-처리)
    * [6. API 실행](#6-api-실행)
  * [JobOperator: 배치 작업의 완전한 제어](#joboperator-배치-작업의-완전한-제어)
    * [JobOperator란?](#joboperator란)
    * [JobOperator 사용 예시](#joboperator-사용-예시)
      * [잡 중단 메커니즘](#잡-중단-메커니즘)
* [Batch Flow](#batch-flow)
  * [Batch Flow란?](#batch-flow란)
    * [Flow 3대 핵심 요소](#flow-3대-핵심-요소)
      * [1) 상태(State)](#1-상태state)
      * [2) 전이 조건 (ExitCode)](#2-전이-조건-exitcode)
      * [3) 전이 규칙 (Transition)](#3-전이-규칙-transition)
    * [Flow 구현 기본 예시](#flow-구현-기본-예시)
  * [커스텀 ExitCode를 활용한 Flow 분기 제어](#커스텀-exitcode를-활용한-flow-분기-제어)
    * [청크 지향 처리에서의 커스텀 ExitCode 설정](#청크-지향-처리에서의-커스텀-exitcode-설정)
    * [커스텀 ExitCode 활용 예시](#커스텀-exitcode-활용-예시)
    * [암시적 전이 규칙](#암시적-전이-규칙)
    * [패턴 매칭 ExitCode](#패턴-매칭-exitcode)
      * [1) `*`](#1-)
      * [2) `?`](#2-)
      * [매칭되는 여러 조건이 있는 경우, 전이 우선순위](#매칭되는-여러-조건이-있는-경우-전이-우선순위)
  * [EndState 설정](#endstate-설정)
    * [`end()` : COMPLETED 상태로 전이](#end--completed-상태로-전이)
    * [`stop()` : STOPPED 상태로 전이](#stop--stopped-상태로-전이)
    * [`fail()` : FAILED 상태로 전이](#fail--failed-상태로-전이)
    * [`end(status)` : 커스텀 EndState로 전이](#endstatus--커스텀-endstate로-전이)
  * [JobExecutionDecider](#jobexecutiondecider)
    * [JobExecutionDecider 사용 예시](#jobexecutiondecider-사용-예시)
  * [Flow를 독립적인 Bean 객체로 만들기](#flow를-독립적인-bean-객체로-만들기)
    * [예시코드) Flow 를 Bean 객체로 만들기](#예시코드-flow-를-bean-객체로-만들기)
    * [예시코드) Flow 재사용방법 1 - Job에 Flow 주입](#예시코드-flow-재사용방법-1---job에-flow-주입)
    * [예시코드) Flow 재사용방법 2 - Flow를 하나의 Step으로 사용](#예시코드-flow-재사용방법-2---flow를-하나의-step으로-사용)
  * [Flow 사용시 주의점](#flow-사용시-주의점)
* [멀티스레드 스텝](#멀티스레드-스텝)
  * [개요](#개요-1)
    * [멀티스레드 스텝 동작 프로세스](#멀티스레드-스텝-동작-프로세스)
    * [멀티쓰레드 스텝에서의 StepExecution 바인딩](#멀티쓰레드-스텝에서의-stepexecution-바인딩)
    * [기본 예시 코드](#기본-예시-코드)
  * [멀티스레드 환경에서의 스레드 안전성](#멀티스레드-환경에서의-스레드-안전성)
    * [데이터 처리 안전성 (데이터 읽고/쓰기 관점에서의 스레드 안전성)](#데이터-처리-안전성-데이터-읽고쓰기-관점에서의-스레드-안전성)
      * [ItemReader 와 SynchronizedItemStreamReader/SynchronizedItemReader](#itemreader-와-synchronizeditemstreamreadersynchronizeditemreader)
      * [ItemWriter 와 SynchronizedItemStreamWriter/SynchronizedItemWriter](#itemwriter-와-synchronizeditemstreamwritersynchronizeditemwriter)
    * [실행 상태 안전성 (메타데이터 기록 동기화 관점에서의 스레드 안전성)](#실행-상태-안전성-메타데이터-기록-동기화-관점에서의-스레드-안전성)
      * [ItemStream 과 실행 상태 안전성](#itemstream-과-실행-상태-안전성)
  * [멀티쓰레드 환경에서의 Job : 실전 예제 코드](#멀티쓰레드-환경에서의-job--실전-예제-코드)
    * [taskExecutor 코드 설명](#taskexecutor-코드-설명)
* [파티셔닝](#파티셔닝)
  * [멀티쓰레드 스텝의 한계와 파티셔닝](#멀티쓰레드-스텝의-한계와-파티셔닝)
  * [파티셔닝 핵심 아키텍처](#파티셔닝-핵심-아키텍처)
  * [핵심 컴포넌트 및 전략](#핵심-컴포넌트-및-전략)
    * [Partitioner](#partitioner)
      * [Partitioner 구현 예시](#partitioner-구현-예시)
      * [파티셔닝 전체 예시 코드](#파티셔닝-전체-예시-코드)
    * [파티션으로 나눈 스텝 내에서 다중 쓰레드 사용](#파티션으로-나눈-스텝-내에서-다중-쓰레드-사용)
    * [파일 기반 Partitioner](#파일-기반-partitioner)
      * [MultiResourcePartitioner 코드 분석](#multiresourcepartitioner-코드-분석)
      * [MultiResourcePartitioner 사용 예시 코드](#multiresourcepartitioner-사용-예시-코드)
* [Spring Batch Test](#spring-batch-test)
  * [Test 를 위한 세팅](#test-를-위한-세팅)
    * [의존성 추가](#의존성-추가)
    * [테스트 전용 H2 DB 구성](#테스트-전용-h2-db-구성)
    * [테스트용 테이블 정의](#테스트용-테이블-정의)
    * [테스트 대상 잡](#테스트-대상-잡)
  * [E2E 테스트 - 전체 배치 잡을 테스트하기](#e2e-테스트---전체-배치-잡을-테스트하기)
  * [단위 테스트 - 개별 컴포넌트 테스트하기](#단위-테스트---개별-컴포넌트-테스트하기)
<!-- TOC -->
